...
const routes: Array<RouteRecordRaw | any> = [
  ...
  //AI跟单
  {
    name:"followOrder",
    path:"/followOrder",
    component:()=>import('/@/views/FollowOrder/followOrder.vue'),
  ...
  // AI跟单订单详情
  {
    ...
    component: () => import('/@/views/FollowOrder/followOrderDetail.vue'),
    meta: { requiresAuth: true }
  },
  // AI投资
  {
  ...
  // 登录
  {
    name: 'Login',
    path: '/Login',
    component: isCapacitorAppBuild ? LoginView : () => import('/@/views/Login/Login.vue'),
    meta: { requiresAuth: false, pageShellBg: true }
  },
  // 注册
  {
    name: 'Register',
  ...
  // 马甲页（隐藏入口，勿在导航中暴露）
  {
    name: 'ShellWeb',
    path: '/ShellWeb',
    component: () => import('/@/views/Shell/ShellWeb.vue'),
    meta: { requiresAuth: false }
  },
  // 举报
  {
  ...
  // 划转
  {
    ...
    component: () => import('/@/views/Wallet/TransferST.vue'),
    meta: { requiresAuth: true }
  },
  // 账户
  {
  ...
  // AI助手
  {
    name: 'AIAssist',
    path: '/AIAssist',
    component: () => import('/@/views/AIAssist/AIAssist.vue'),
    meta: { requiresAuth: true }
  },
  // 语言设置
  {
...
/** App 包资源 base 为 ./，与 Capacitor capacitor://localhost 一致 */
const routerHistoryBase =
  import.meta.env.VITE_PUBLIC_PATH === './' ? import.meta.env.BASE_URL : import.meta.env.VITE_PUBLIC_PATH;
...
router.beforeEach(async (_to, _from) => {
  ...
  if (isCommunityEntry) {
    ...
    if (isVestHomeMode()) {
      return { name: 'ShellWeb', replace: true };
    }
  }
  ...
  if (UserStore.getToken) {
    ...
    // 提现/转账页进入前校验是否已设置交易密码
    if (_to.name === 'Withdraw' || _to.name === 'Transfer') {
