...
/** 回滚到内置 bundle（Capgo `builtin`），用于 OTA 包损坏导致白屏 */
export const resetOtaToBuiltinBundle = async (): Promise<boolean> => {
  ...
  try {
    ...
    console.warn('[resetOtaToBuiltinBundle]', e);
    return false;
  }
};
...
export const initAppUpdateInstallLifecycleListeners = (): void => {
  ...
  void CapacitorUpdater.addListener('set' as any, (evt: { bundle?: { id?: string } }) => {
