...
export class WebSocketClient {
  ...
  private clearVisibilityReconnectHandler(): void {
    if (this.visibilityReconnectHandler && typeof document !== 'undefined') {
      document.removeEventListener('visibilitychange', this.visibilityReconnectHandler);
    ...
    if (typeof document !== 'undefined' && document.visibilityState === 'hidden') {
      if (!this.visibilityReconnectHandler) {
        this.visibilityReconnectHandler = () => {
          if (this.isManualClose || document.visibilityState !== 'visible') return;
          this.clearVisibilityReconnectHandler();
          this.tryReconnect();
        };
        document.addEventListener('visibilitychange', this.visibilityReconnectHandler);
      }
      return;
    }
