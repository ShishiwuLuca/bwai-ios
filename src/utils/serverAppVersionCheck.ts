...
export const checkServerAppVersion = async (
  ...
  try {
    ...
    if (tryResource) {
      // 启动后静默检测不得自动 set OTA：坏包会持久化，表现为进后台/重启后整页空白
      if (silent) {
        console.log(
          '[checkServerAppVersion] silent: skip auto OTA',
          normalized.resourceVersion
        );
      } else {
    ...
    if (!silent) {
      ...
      console.log('[checkServerAppVersion] silent: update available, dialog skipped');
    }
    const notifyTitle = [
      translate('app_version_new_available'),
