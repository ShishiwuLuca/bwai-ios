/** Apple 硬件标识符 → 市场名称（数据源自 react-native-device-info 社区维护映射） */
export const APPLE_HARDWARE_MARKETING_NAMES: Record<string, string> = {
  'iPhone1,1': 'iPhone',
  'iPhone1,2': 'iPhone 3G',
...
/** getAppleHardwareMarketingName */
export const getAppleHardwareMarketingName = (deviceId: string): string => {
  const id = deviceId.trim();
  if (!id) return '';
  return APPLE_HARDWARE_MARKETING_NAMES[id] ?? '';
};
