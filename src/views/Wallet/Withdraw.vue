<template>
  <div class="withdraw-page">
    ...
    <PageWrap>
      <div class="flex flex-col gap-[0.32rem] p-[0.32rem] mb-10">
        <div class="flex flex-col gap-[0.16rem]">
          <label class="text-[0.28rem] text-[#000] font-medium">{{ t('str_withdraw_currency') }}</label>
          <Popover v-model:show="showCurrencyPopover" :offset="[0, 8]" placement="bottom">
            <template #reference>
              <Cell
                ...
                class="!rounded-sm withdraw-page__cell"
              >
              <template #icon>
                <div class="w-[30px] h-[30px] shrink-0 mr-1 rounded-full overflow-hidden">
                  <Icon :name="SelectedNetworkCoin.assetIcon" :size="30" color="var(--van-primary-color)" />
                </div>
              </template>
            </Cell>
          </template>
          <div class="w-28 max-h-17 !bg-white overflow-auto p-0.5 border-1 border-solid border-[#e0e1fd] rounded-sm">
            <Cell
            ...
            >
              <template #icon>
                <div class="w-[30px] h-[30px] shrink-0 mr-1 rounded-full overflow-hidden">
                  <Icon :name="item.assetIcon" :size="30" color="var(--van-primary-color)" />
                </div>
                </template>
                <template #right-icon>
                  <Radio shape="dot" :icon-size="16" checked-color="#2071f8" :checked="SelectedNetworkCoin.assetCode === item.assetCode" :name="item.assetCode" />
                </template>
              </Cell>
            </div>
          </Popover>
        ...
        <div class="flex flex-col gap-[0.16rem]">
          <label class="text-[0.28rem] text-[#000] font-medium">{{ t('str_withdraw_chain') }}</label>
          <Popover v-model:show="showNetworkPopover" :offset="[0, 8]" placement="bottom">
            ...
            <div class="w-28 max-h-17 !bg-white overflow-auto p-0.5 border-1 border-solid border-[#e0e1fd] rounded-sm">
              ...
              >
                <template #right-icon>
                  <Radio shape="dot" :icon-size="16" checked-color="#2071f8" :checked="SelectedNetworkType.chainTypeCode === item.chainTypeCode" :name="item.chainTypeCode" />
                </template>
              </Cell>
            </div>
          </Popover>
        ...
        <div class="flex flex-col gap-[0.16rem]">
          <label class="text-[0.28rem] text-[#000] font-medium">{{ t('str_withdraw_address') }}</label>
          <Field
            clickable
            center
            clearable
          ...
          >
            <template #button>
              <div class="text-[#2071f8]" @click="showAddressBookPopup = true">{{ t('address_book_title') }}</div>
            </template>
          </Field>
        </div>
        <div class="flex flex-col gap-[0.16rem]">
          <label class="text-[0.28rem] text-[#000] font-medium">{{ t('str_withdraw_amount') }}</label>
          <Field
            clickable
            center
            clearable
          ...
          >
            <template #button>
              <div class="text-[#2071f8]">USDT | <span @click="SaveAll">{{ t('common_action_sendall') }}</span></div>
            </template>
          </Field>
          <div class="flex items-center justify-between">
            <div class="text-[#333] text-[0.25rem]">{{ t('str_available_Lpbalance') }}</div>
            <div class="text-[#2071f8] text-[0.25rem]">{{ AccountBalance.balance }} USDT</div>
          </div>
        </div>
        <div class="flex flex-col gap-[0.16rem]">
          ...
          <div class="withdraw-page__tips rounded-sm overflow-hidden p-1 mt-2">
            <label class="text-[0.28rem] text-[#000] font-medium">{{ t('str_recharge_tips') }}</label>
            <div class="text-[0.24rem] text-[#8c8c8c]">
              <div>{{ t('str_withdraw_tips', [AccountBalance.withdrawMin, AccountBalance.withdrawMax, 'USDT']) }}</div>
      ...
      <div class="fixed bottom-0 left-0 right-0 p-1 bg-[#f2f5fe]">
        <div class="flex items-center justify-between pb-0.5">
          <div class="text-[#333]">{{ t('str_withdraw_fee_tips') }}</div>
          <div class="text-[#2071f8]">{{ CurrentWithdrawFeeRule.fee_val }} {{ CurrentWithdrawFeeRule.mode === 'fixed' ? 'USDT' : '%' }}</div>
        </div>
        <div class="flex items-center justify-between pb-0.5">
          <div class="text-[#333]">{{ t('str_withdraw_actual_amount') }}</div>
          <div class="text-[#2071f8]">{{ ReturnPrecision(ActualWithdrawAmount, 2) }} USDT</div>
        </div>
        <Button v-if="isLoading" :loading="isLoading" block round class="withdraw-page__submit-btn" />
        <Button v-else block round class="withdraw-page__submit-btn" @click="onSubmit">
    ...
    <Popup
      ...
      position="bottom"
      class="w-full p-1 !max-h-[90%] !min-h-[50%]"
      :style="{ background: '#fff' }"
    >
      <div class="flex items-center justify-between pb-1">
        <div class="text-[0.3rem] font-bold text-[#000]">{{ t('address_book_placeholder') }}</div>
        <div @click="showAddressBookPopup = false">
          <Icon name="cross" :size="25" color="#000" />
        </div>
      </div>
      <PullRefresh v-model="refreshing" @refresh="onRefresh" class="!overflow-auto !max-h-[87%]" :style="{ minHeight: '40vh' }">
        <List v-model:loading="loading" :finished="finished" :immediate-check="false" @load="onLoad">
          <template #finished>
            ...
            <Button
              ...
              block
              round
              class="!mx-auto !w-[4rem] withdraw-page__submit-btn"
          ...
          <Cell
            ...
            size="large"
            :border="false"
            class="rounded !pt-1 !pb-1 mb-1 withdraw-page__cell"
            title-class="!text-[#000] !font-medium"
...
<script setup lang="ts">
  ...
  import PasswordEyeToggle from '/@/components/PasswordEyeToggle/PasswordEyeToggle.vue';
